...
import { asCueId, asTimelinePosition, asTrackId } from './types'
...
const tracks: ReadonlyArray<CueTrack> = [
  {
    id: asTrackId('lyrics'),
    cues: [{ id: sharedCueId }, { id: asCueId('verse') }],
  },
  {
    id: asTrackId('scenes'),
    cues: [{ id: sharedCueId }, { id: asCueId('close-up') }],
  },
]
...
describe('multi-track alignment', () => {
  ...
  it('keeps each track state independent when switching tracks', () => {
    ...
    expect(stateResult).toBeSuccess((initialState) => {
      ...
      expect(lyricsState).toBeDefined()
      ...
      expect(markResult).toBeSuccess((nextLyricsState) => {
        const statesByTrackId = new Map(initialState.statesByTrackId)
        statesByTrackId.set(lyrics.id, nextLyricsState)
        ...
        expect(switchResult).toBeSuccess((switchedState) => {
          ...
          expect(
            switchedState.statesByTrackId
              .get(asTrackId('lyrics'))
              ?.marksByCueId.get(sharedCueId),
          ).toBe(1.25)
          expect(
            switchedState.statesByTrackId.get(asTrackId('scenes'))?.marksByCueId
              .size,
          ).toBe(0)
  ...
  it('rejects duplicate and unknown track IDs', () => {
    ...
    expect(
      createMultiTrackAlignmentState({ tracks: duplicateTracks }),
    ).toBeFailure((error) => {
      expect(error).toBeInstanceOf(DuplicateTrackIdError)
    })

    const stateResult = createMultiTrackAlignmentState({ tracks })
    expect(stateResult).toBeSuccess((state) => {
      expect(selectTrack(state, tracks, asTrackId('missing'))).toBeFailure(
        (error) => {
